...
def train(args, model, train_dataloader, test_dataloader, optimizer,
    ...
    for epoch in range(epochs):
        ...
        if mae <= best_mae and mae < 20:     # 误差最大为20, 小于20的保存下来
            ...
            # checkpoint_dir已在函数开始时创建，这里不需要重复创建
    ...
    if best_model_state is not None:
        ...
        # 使用最佳模型进行完整验证并收集预测值
        print(f"使用第{best_epoch+1}轮的最佳模型收集预测值...")
        mae_final, rmse_final, sd_final, all_predictions, all_labels = val(
        ...
        return best_mae, all_predictions, all_labels
    else:
        print("警告：未找到最佳模型，无法收集预测值")
        return best_mae, [], []


# 对训练好的模型进行验证，计算模型在测试数据集上的平均绝对误差（MAE）和均方根误差（RMSE）
def val(test_dataloader, test_fold, model, checkpoint_dir=None, epoch=0, return_predictions=False):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (image, pca_image, time_str, para, label) in enumerate(test_bar):
            ...
            if return_predictions:
                all_predictions.extend(predict.cpu().numpy().tolist())
    ...
    if return_predictions:
        return mae, rmse, sd, all_predictions, all_labels
    else:
        return mae, rmse, sd
...
def main(args, test_fold=1):
    ...
    best_mae, all_predictions, all_labels = train(args=args,
          model=model,
...
if __name__ == '__main__':
    ...
    k_fold = arg_s.k_fold
    arg_s.save_model_path = r'/mnt/500T/homes/lm/CME-Project/MODEL/gcn_3'  # 模型结果保存路径
    ...
    #     best_mae, all_predictions, all_labels = main(args=arg_s, test_fold=int(i + 1))
    #     fold_results.append({
    #         'fold': i + 1,
    #         'best_mae': best_mae,
    ...
    #         'labels': all_labels
    #     })
    #     print(f"第{i+1}折完成，最佳MAE: {best_mae}")
    
    # # 找到最好的一折（MAE最小的）
    # best_fold_result = min(fold_results, key=lambda x: x['best_mae'])
    # best_fold = best_fold_result['fold']
    # best_predictions = best_fold_result['predictions']
    
    # print(f"\n最好的一折是第{best_fold}折，MAE: {best_fold_result['best_mae']}")
    # print(f"预测值数量: {len(best_predictions)}")
    
    # # 保存最好一折的所有预测值到txt文件
    # output_dir = arg_s.save_model_path
    # if not os.path.exists(output_dir):
    #     os.makedirs(output_dir)
    
    # output_file = os.path.join(output_dir, f'best_fold_{best_fold}_predictions.txt')
    # with open(output_file, 'w', encoding='utf-8') as f:
    #     f.write(f"最佳折: 第{best_fold}折\n")
    #     f.write(f"最佳MAE: {best_fold_result['best_mae']}\n")
    #     f.write(f"预测值总数: {len(best_predictions)}\n")
    #     f.write("=" * 50 + "\n")
    #     f.write("预测值列表:\n")
    #     for idx, pred in enumerate(best_predictions):
    #         f.write(f"{idx + 1}: {pred:.6f}\n")
    
    # print(f"预测值已保存到: {output_file}")

    for i in range(k_fold):
        fold_id = i + 1

        best_mae, best_predictions, best_labels = main(args=arg_s, test_fold=int(fold_id))
        ...
        print(f"第{fold_id}折完成，最佳MAE: {best_mae}")
        print(f"样本数: {len(best_predictions)}")

        # ====== 每一折单独保存 best epoch 的预测值+真实值 ======
        output_file = os.path.join(output_dir, f'fold_{fold_id}_best_preds_labels.txt')
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"折数: 第{fold_id}折\n")
            f.write(f"该折最佳MAE: {best_mae}\n")
            f.write(f"样本总数: {len(best_predictions)}\n")
            f.write("=" * 60 + "\n")
            f.write("index\tprediction\tlabel\n")

            for idx, (pred, label) in enumerate(zip(best_predictions, best_labels)):
                f.write(f"{idx + 1}\t{float(pred):.6f}\t{float(label):.6f}\n")

        print(f"第{fold_id}折 best 预测值+真实值 已保存到: {output_file}")
